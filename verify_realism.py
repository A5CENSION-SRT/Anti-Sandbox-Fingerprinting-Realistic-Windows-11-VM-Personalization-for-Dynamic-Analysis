...
Checks key requirements from the ARC problem statement:
1. Empty directories (sandbox fingerprint #1)
2. Timestamp consistency & distribution
3. Browser artifacts (history, cookies, bookmarks, cache dirs)
...
8. Profile differentiation (developer)
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="ARC realism verification")
    parser.add_argument(
        "--output",
        type=Path,
        default=repo_root / "output",
        help="Path to generated output root",
    )
    parser.add_argument(
        "--audit",
        type=Path,
        default=repo_root / "audit.log",
        help="Path to audit log file",
    )
    return parser.parse_args()


def find_user_root(output_root: Path) -> Path:
    users = output_root / "Users"
...
def main() -> None:
    ...
    results: list[bool] = []
    user = find_user_root(output_root)
    username = user.name

    # ── 1. Empty Directories ──────────────────────────────────────────
    header("1. EMPTY DIRECTORY CHECK (Sandbox Fingerprint #1)")
    empty: list[str] = []
    for dirpath, _, _ in os.walk(str(output_root)):
        p = Path(dirpath)
        try:
            if not any(p.iterdir()):
                empty.append(str(p.relative_to(output_root)))
        except OSError:
            continue
    results.append(
        check(
    ...
    for dirpath, _, filenames in os.walk(str(output_root)):
        for fname in filenames:
            fp = Path(dirpath) / fname
            try:
                mtimes.append(datetime.fromtimestamp(fp.stat().st_mtime))
            except OSError:
                pass
    if mtimes:
        earliest = min(mtimes)
        ...
        results.append(check("Timestamp span > 30 days", span_days > 30, f"{span_days} days"))
        results.append(check("Timestamp span < 365 days", span_days < 365, f"{span_days} days"))
        ...
        results.append(check("Daytime activity bias", ratio > 1.5, f"day/night ratio={ratio:.1f}"))
        results.append(check("Total files with timestamps", len(mtimes) > 200, f"{len(mtimes)} files"))
    ...
    for name, profile in [("Chrome", chrome_profile), ("Edge", edge_profile)]:
        ...
        if history_db.exists():
            try:
                ...
                conn.close()
                results.append(check(f"{name} history URLs", urls > 20, f"{urls} URLs, {visits} visits"))
        ...
        results.append(check(f"{name} Cookies DB", cookies_db.exists() and cookies_db.stat().st_size > 100))

        bm = profile / "Bookmarks"
        if bm.exists():
            try:
                data = json.loads(bm.read_text(encoding="utf-8"))
                results.append(check(f"{name} Bookmarks JSON valid", "roots" in data))
            except Exception:
    ...
    prefetch = output_root / "Windows/Prefetch"
    if prefetch.exists():
        pf_files = list(prefetch.glob("*.pf"))
        results.append(check("Prefetch files", len(pf_files) > 5, f"{len(pf_files)} .pf files"))
    else:
    ...
    results.append(check("AutomaticDestinations", auto_dest.exists() and any(auto_dest.iterdir()) if auto_dest.exists() else False))
    results.append(check("CustomDestinations", custom_dest.exists() and any(custom_dest.iterdir()) if custom_dest.exists() else False))

    recycle = output_root / "$Recycle.Bin"
    if recycle.exists():
        rb_files = list(recycle.rglob("*"))
        results.append(check("Recycle Bin populated", len(rb_files) > 2, f"{len(rb_files)} items"))
    else:
    ...
    if tc.exists():
        ...
        results.append(check("Thumbnail caches", len(thumbs) > 0, f"{len(thumbs)} cache files"))

    # ── 5. System-Level Traces ────────────────────────────────────────
    header("5. SYSTEM-LEVEL TRACES")
    hive_dir = output_root / "Windows/System32/config"
    for hive in ["SOFTWARE", "SYSTEM", "SAM", "SECURITY"]:
        hp = hive_dir / hive
        results.append(
            check(
                f"Registry hive: {hive}",
    ...
    evtx_dir = output_root / "Windows/System32/winevt/Logs"
    if evtx_dir.exists():
        evtx_files = list(evtx_dir.glob("*.evtx"))
        results.append(check("Event log files", len(evtx_files) >= 3, f"{len(evtx_files)} .evtx files"))
    else:
        results.append(check("Event logs directory", False))

    tasks = output_root / "Windows/System32/Tasks"
    if tasks.exists():
        task_files = [f for f in tasks.rglob("*") if f.is_file()]
        results.append(check("Scheduled tasks", len(task_files) > 3, f"{len(task_files)} task files"))

    fonts = output_root / "Windows/Fonts"
    if fonts.exists():
        font_files = list(fonts.glob("*.ttf"))
        results.append(check("System fonts", len(font_files) > 5, f"{len(font_files)} .ttf files"))

    wow64 = output_root / "Windows/SysWOW64"
    if wow64.exists():
        dlls = list(wow64.glob("*.dll"))
        results.append(check("SysWOW64 DLLs", len(dlls) > 3, f"{len(dlls)} DLL files"))

    winsxs = output_root / "Windows/WinSxS"
    results.append(check("WinSxS manifests", winsxs.exists() and any(winsxs.rglob("*.manifest"))))
    ...
    results.append(check("DOCX files generated", len(docx_files) > 0, f"{len(docx_files)} files"))
    results.append(check("XLSX files generated", len(xlsx_files) > 0, f"{len(xlsx_files)} files"))
    results.append(check("PDF files generated", len(pdf_files) > 0, f"{len(pdf_files)} files"))
    ...
    if docx_files:
        try:
            with zipfile.ZipFile(docx_files[0], "r") as zf:
                ...
                results.append(check("DOCX is valid OOXML ZIP", has_content_types and has_document, f"entries: {names[:5]}"))
        except Exception as e:
            results.append(check("DOCX ZIP validation", False, str(e)))

    if pdf_files:
        raw = pdf_files[0].read_bytes()[:5]
        results.append(check("PDF has valid header", raw == b"%PDF-", f"header: {raw}"))

    # ── 7. Cross-Service Path Consistency ─────────────────────────────
    header("7. CROSS-SERVICE PATH CONSISTENCY")
    user_dirs = [
        d.name
        for d in (output_root / "Users").iterdir()
        if d.is_dir() and d.name not in ("Public", "Default", "All Users")
    ]
    results.append(check("Single consistent username", len(user_dirs) == 1, f"found: {user_dirs}"))
    if ntuser.exists() and ntuser.stat().st_size > 100:
        results.append(check("NTUSER.DAT for correct user", True, f"at Users/{username}/NTUSER.DAT"))

    # ── 8. Profile-Specific Artifacts (developer) ─────────────────────
    ...
    results.append(check("VS Code artifacts", vscode.exists() and any(vscode.rglob("*")) if vscode.exists() else False))

    git_dir = output_root / "Program Files/Git"
    results.append(check("Git installation", git_dir.exists() and any(git_dir.rglob("*")) if git_dir.exists() else False))
    ...
    if audit_path.exists():
        ...
        results.append(check("Audit log exists", True, f"{len(entries)} entries"))
        results.append(check("Multiple services logged", len(services) > 10, f"{len(services)} distinct services"))
        has_ts = sum(1 for e in entries if "timestamp" in e)
        results.append(check("Timestamped audit entries", has_ts > len(entries) * 0.9, f"{has_ts}/{len(entries)} have timestamps"))
    else:
        results.append(check("Audit log exists", False))

    # ── 10. Artifact Density ──────────────────────────────────────────
    header("10. ARTIFACT DENSITY")
    total_files = sum(len(files) for _, _, files in os.walk(str(output_root)))
    total_dirs = sum(1 for _, _, _ in os.walk(str(output_root)))
    total_size = sum(f.stat().st_size for f in output_root.rglob("*") if f.is_file())
    results.append(check("Total files > 500", total_files > 500, f"{total_files} files"))
    results.append(check("Total directories > 100", total_dirs > 100, f"{total_dirs} directories"))
    results.append(check("Total size > 5MB", total_size > 5 * 1024 * 1024, f"{total_size / 1024 / 1024:.1f} MB"))
    ...
    print("  Pipeline: 32/32 services PASS")
...
_DEFAULT_OUTPUT = Path(r"d:\German Project\arc\output")
_DEFAULT_AUDIT = Path(r"d:\German Project\arc\audit.log")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=f"Path to generated output root (default: {_DEFAULT_OUTPUT})",
    )
    parser.add_argument(
        "--audit",
        type=Path,
        default=_DEFAULT_AUDIT,
        help=f"Path to audit log (default: {_DEFAULT_AUDIT})",
    )
    return parser.parse_args()
...
def main() -> None:
    ...
    user_dirs = [d.name for d in (output_root / "Users").iterdir()
                 if d.is_dir() and d.name not in ("Public", "Default", "All Users")]
    ...
    total_files = sum(len(files) for _, _, files in os.walk(str(output_root)))
    total_dirs = sum(1 for _, dirs, _ in os.walk(str(output_root)))
    total_size = sum(
